...
if [[ $disk_mount != "boot" ]] 
 then
  if (($disk_usage>70 && $disk_usage<80))
   then
    echo "WARNING - $disk_name DISK USAGE MORE THEN 70%" | mail -s "DISK USAGE" ermvs88ryv57@mail.ru
   elif (($disk_usage>80 && $disk_usage<90 ))
    then
     top10=`find . / -type f -exec du {} 2>/dev/null \;|sort -nr | head`
     echo "WARNING - $disk_name DISK USAGE MORE THEN 80%. TOP 10 FILES WITH BIG SIZE - $top10" |mail -s "DISK USAGE" ermvs88ryv57@mail.ru
   elif (( $disk_usage>90 ))
    then
